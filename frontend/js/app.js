...
// ── PIN DE SEGURANÇA ──
let _pinAtual = '';
let _pinModo = 'verificar'; // verificar | definir | confirmar
let _pinTemp = '';
...
async function verificarPinAoEntrar() {
  ...
  try {
    ...
    if (!r.tem) return true; // sem PIN, pode entrar
    // Tem PIN — mostrar tela
...
async function processarPin() {
  if (_pinModo === 'verificar') {
    try {
      const r = await api('POST', '/api/membros/pin/verificar', {
        membro_id: APP.membroId, pin: _pinAtual
      });
      if (r.ok) {
...
async function abrirConfigPin() {
  ...
  document.getElementById('config-pin-form').style.display = 'none';
...
function iniciarAlterarPin() {
  iniciarDefinirPin();
}

async function removerPin() {
  if (!confirm('Remover o PIN de segurança?')) return;
  try {
